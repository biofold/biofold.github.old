...
;(function (Jmol) {
	...
	Jmol._jsGetMouseModifiers = function(ev) {
		...
		switch (ev.button) {
		case 0:
		  modifiers = J.api.Event.MOUSE_LEFT;
		  break;
		case 1:
		  modifiers = J.api.Event.MOUSE_MIDDLE;
		  break;
		case 2:
		  modifiers = J.api.Event.MOUSE_RIGHT;
		  break;
		}
		if (ev.shiftKey)
		  modifiers += J.api.Event.SHIFT_MASK;
		if (ev.altKey)
		  modifiers += J.api.Event.ALT_MASK;
		if (ev.ctrlKey)
		  modifiers += J.api.Event.CTRL_MASK;
		return modifiers;
	}
	...
	Jmol._jsSetMouse = function(canvas) {
      ...
      if ((ev.type == "touchstart") && Jmol._gestureUpdate(canvas, ev))
        ...
        return false;
			...
			canvas.applet._applet.viewer.handleOldJvm10Event(J.api.Event.MOUSE_DOWN,xym[0],xym[1],xym[2],System.currentTimeMillis());
			return false;
		});
      ...
      //System.out.println(canvas.isDragging + " " + ev.type + xym[0] + " " + xym[1] + " " + xym[2] + " " + System.currentTimeMillis())
			...
			canvas.applet._applet.viewer.handleOldJvm10Event(
				(canvas.isDragging ? J.api.Event.MOUSE_DRAG : J.api.Event.MOUSE_MOVE),
      ...
      if (ev.type == "touchend" && Jmol._gestureUpdate(canvas, ev))
        return false;
			var xym = Jmol._jsGetXY(canvas, ev);
			if(!xym) return false;
			canvas.applet._applet.viewer.handleOldJvm10Event(J.api.Event.MOUSE_UP,xym[0],xym[1],xym[2],System.currentTimeMillis());
			return false;
...
//				Math.floor($(Jmol._getElement(this, "appletdiv")).height()));
  ...
  Jmol._repaint = function(applet, asNewThread) {
    ...
    //
    // requires rebuilding via methods of classes in J.exportjs, which will come back from updateJS
