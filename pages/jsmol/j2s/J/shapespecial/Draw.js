...
Clazz.load (["java.lang.Enum", "J.shape.MeshCollection", "J.util.P3i", "$.V3"], "J.shapespecial.Draw", ["java.lang.Boolean", "$.Float", "java.util.ArrayList", "$.Hashtable", "J.shapespecial.DrawMesh", "J.util.ArrayUtil", "$.BS", "$.BSUtil", "$.C", "$.Escape", "$.Logger", "$.Measure", "$.MeshSurface", "$.P3", "$.SB", "$.TextFormat"], function () {
...
Clazz.superCall (this, J.shapespecial.Draw, "setProperty", [propertyName, value, bs]);
...
for (var i = 0, pt = 0; i < n; i++) this.lineData.add ([J.util.P3.new3 (fdata[pt++], fdata[pt++], fdata[pt++]), J.util.P3.new3 (fdata[pt++], fdata[pt++], fdata[pt++])]);
...
this.vData.add ([Integer.$valueOf (1), J.util.P3.new3 (NaN, NaN, NaN)]);
return;
...
return (data[2] != null);
}return Clazz.superCall (this, J.shapespecial.Draw, "getPropertyData", [property, data]);
}, "~S,~A");
...
return Clazz.superCall (this, J.shapespecial.Draw, "getProperty", [property, index]);
}, "~S,~N");
...
if (!isPickingMode && !isDrawPicking && !isSpinMode || J.util.C.isColixTranslucent (this.colix)) return null;
...
for (var i = 0; i < this.meshCount; i++) {
var m = this.meshes[i];
if (key == null || J.util.TextFormat.isMatch (m.thisID.toUpperCase (), key, true, true)) sb.append (this.getDrawCommand (m, m.modelIndex));
}
return sb.toString ();
}, $fz.isPrivate = true, $fz), "J.shapespecial.DrawMesh");
Clazz.defineMethod (c$, "getDrawCommand", 
...
if (mesh.connections != null) str.append (" connect ").append (J.util.Escape.e (mesh.connections));
break;
...
for (var i = 0; i < mesh.polygonCount; i++) if (mesh.polygonIndexes[i] == null) str.append (" [0 0 0 0]");
 else str.append (" ").append (J.util.Escape.escapeAI (mesh.polygonIndexes[i]));
...
info.put ("vertices", v);
if (mesh.drawType === J.shapespecial.Draw.EnumDrawType.LINE) info.put ("length_Ang",  new Float (mesh.vertices[0].distance (mesh.vertices[1])));
}V.add (info);
}
return V;
});
