...
Clazz.load (null, "J.io2.XmlReader", ["java.lang.Character", "J.io.XmlUtil", "J.util.Point3f", "$.StringXBuilder"], function () {
...
data = sb.toString ();
}return J.io2.XmlReader.extractTag (data, tag, closer, withTag);
}, "~S,~S,~B,~B");
c$.extractTagOnly = Clazz.defineMethod (c$, "extractTagOnly", 
function (data, tag) {
return J.io2.XmlReader.extractTag (data, "<" + tag + ">", "</" + tag + ">", false);
}, "~S,~S");
...
var pt = J.io2.XmlReader.setNext (data, what, nexta, 1);
if (pt < 2 || (pt = J.io2.XmlReader.setNext (data, "\"", nexta, 0)) < 2) return "";
var pt1 = J.io2.XmlReader.setNext (data, "\"", nexta, -1);
return (pt1 <= 0 ? "" : data.substring (pt, pt1));
}, "~S,~S");
Clazz.defineMethod (c$, "getXmlPoint", 
function (data, key) {
var spt = J.io2.XmlReader.getXmlAttrib (data, key).$replace ('(', '{').$replace (')', '}');
var value = J.util.Escape.unescapePoint ();
if (Clazz.instanceOf (value, J.util.Point3f)) return value;
return  new J.util.Point3f ();
}, "~S,~S");
...
ipt += what.length;
next[0] = ipt + offset;
if (offset > 0 && ipt < data.length && data.charAt (ipt) != '=') return J.io2.XmlReader.setNext (data, what, next, offset);
return next[0];
