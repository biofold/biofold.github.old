...
Clazz.load (["java.lang.Float", "J.util.BS", "$.V3"], "J.modelset.AtomCollection", ["java.lang.Character", "java.util.ArrayList", "$.Arrays", "J.atomdata.RadiusData", "J.constant.EnumPalette", "$.EnumStructure", "$.EnumVdw", "J.geodesic.EnvelopeCalculation", "J.modelset.Group", "$.LabelToken", "J.script.T", "J.util.ArrayUtil", "$.AxisAngle4f", "$.BSUtil", "$.Elements", "$.Escape", "$.Logger", "$.Matrix3f", "$.Measure", "$.P3", "$.Parser", "$.TextFormat", "J.viewer.JC"], function () {
...
nTotal[0] = nH;
return hAtoms;
}, "J.util.BS,~A,~B,~B,java.util.List");
...
var seqcode = J.modelset.Group.getSeqcode (seqNumber, insertionCode);
...
var bs =  new J.util.BS ();
var seqNum = J.modelset.Group.getSequenceNumber (seqcode);
var haveSeqNumber = (seqNum != 2147483647);
var isEmpty = true;
var insCode = J.modelset.Group.getInsertionCode (seqcode);
switch (insCode) {
case '?':
for (var i = this.atomCount; --i >= 0; ) {
var atomSeqcode = this.atoms[i].getSeqcode ();
if (!haveSeqNumber || seqNum == J.modelset.Group.getSequenceNumber (atomSeqcode) && J.modelset.Group.getInsertionCodeValue (atomSeqcode) != 0) {
bs.set (i);
isEmpty = false;
}}
break;
...
for (var i = this.atomCount; --i >= 0; ) {
var atomSeqcode = this.atoms[i].getSeqcode ();
if (seqcode == atomSeqcode || !haveSeqNumber && seqcode == J.modelset.Group.getInsertionCodeValue (atomSeqcode) || insCode == '*' && seqNum == J.modelset.Group.getSequenceNumber (atomSeqcode)) {
bs.set (i);
isEmpty = false;
}}
}
