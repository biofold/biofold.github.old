...
Clazz.load (["java.util.Hashtable", "J.util.BS", "J.viewer.JC"], "J.modelset.ModelLoader", ["java.lang.Boolean", "$.Float", "$.NullPointerException", "java.util.ArrayList", "$.Arrays", "J.api.Interface", "J.constant.EnumStructure", "J.modelset.Chain", "$.Group", "$.Model", "$.ModelSet", "J.util.ArrayUtil", "$.BSUtil", "$.Elements", "$.JmolMolecule", "$.Logger", "$.P3", "$.SB", "$.TextFormat", "$.V3"], function () {
...
this.shapes = info.remove ("shapes");
if (this.shapes != null) this.doAddHydrogens = false;
...
this.freeze ();
this.finalizeShapes (this.shapes);
...
var nRead = 0;
var models = this.modelSet.models;
if (this.modelSet.modelCount > 0) this.nullGroup =  new J.modelset.Group ( new J.modelset.Chain (this.modelSet.models[this.baseModelIndex], ' '), "", 0, -1, -1);
...
this.seqcodes[this.groupCount] = J.modelset.Group.getSeqcode (groupSequenceNumber, groupInsertionCode);
++this.groupCount;
...
if (haveMultipleBonds && this.modelSet.someModelsHaveSymmetry && !this.viewer.getApplySymmetryToBonds ()) J.util.Logger.info ("ModelSet: use \"set appletSymmetryToBonds TRUE \" to apply the file-based multiple bonds to symmetry-generated atoms.");
...
var startSeqCode = J.modelset.Group.getSeqcode (startSequenceNumber, startInsertionCode);
var endSeqCode = J.modelset.Group.getSeqcode (endSequenceNumber, endInsertionCode);
var models = this.modelSet.models;
...
if (group == null) {
group =  new J.modelset.Group (chain, group3, seqcode, firstAtomIndex, lastAtomIndex);
key = "o>";
} else {
...
for (var i = 0; i < shapeSettings.size (); i++) {
var ss = shapeSettings.get (i);
ss.offset (this.baseModelIndex, this.baseAtomIndex);
ss.createShape (this.modelSet);
}
