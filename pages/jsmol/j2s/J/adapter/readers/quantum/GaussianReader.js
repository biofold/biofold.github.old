...
Clazz.load (["J.adapter.readers.quantum.MOReader"], "J.adapter.readers.quantum.GaussianReader", ["java.lang.Character", "$.Exception", "$.Float", "java.util.ArrayList", "$.Hashtable", "J.adapter.smarter.SmarterJmolAdapter", "J.api.JmolAdapter", "J.util.ArrayUtil", "$.Escape", "$.Logger", "$.Parser", "$.TextFormat", "$.V3"], function () {
...
slater[3] = nGaussians;
if (J.util.Logger.debugging) J.util.Logger.info ("Slater " + this.shells.size () + " " + J.util.Escape.e (slater));
...
tokens = this.getTokens ();
if (J.util.Logger.debugging) J.util.Logger.info ("Gaussians " + (i + 1) + " " + J.util.Escape.e (tokens));
gdata.add (tokens);
}
}
}
...
this.gaussianCount += nGaussians;
for (var i = 0; i < nGaussians; i++) {
gdata.add (J.adapter.smarter.AtomSetCollectionReader.getTokensStr (this.readLine ()));
}
}
...
var sym;
if (isNOtype) {
mos[i].put ("occupancy",  new Float (J.util.Parser.parseFloatStr (tokens[i + 2])));
} else {
sym = tokens[i];
