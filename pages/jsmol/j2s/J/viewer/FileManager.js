...
Clazz.load (["java.util.Hashtable"], "J.viewer.FileManager", ["java.io.BufferedInputStream", "$.ByteArrayInputStream", "java.net.URL", "$.URLEncoder", "java.util.ArrayList", "J.api.Interface", "J.io.Base64", "$.FileReader", "$.JmolBinary", "J.util.Escape", "$.Logger", "$.SB", "$.TextFormat", "J.viewer.DataManager", "$.Viewer"], function () {
...
($fz = function (data) {
var reader = (Clazz.instanceOf (data, String) ? "String" : J.util.Escape.isAS (data) ? "Array" : Clazz.instanceOf (data, java.util.List) ? "List" : null);
...
if (localPath != null) script = J.viewer.FileManager.setScriptFileReferences (script, localPath, true);
if (remotePath != null) script = J.viewer.FileManager.setScriptFileReferences (script, remotePath, false);
