...
Clazz.load (["J.util.Int2IntHash"], "J.util.C", ["java.lang.Float", "$.IndexOutOfBoundsException", "J.constant.EnumPalette", "J.util.ArrayUtil", "$.ColorUtil", "$.Escape", "$.Logger", "$.Parser", "$.SB"], function () {
...
for (var i = J.util.C.colixMax; --i >= 4; ) if (argb == J.util.C.argbs[i]) return i;
...
if (colix == 0) colix = 1;
colix &= -30721;
return (isTranslucent ? J.util.C.applyColorTranslucencyLevel (colix, translucentLevel) : colix);
}, "~N,~B,~N");
...
function (argb) {
var a = (argb >> 24) & 0xFF;
if (a == 0xFF) return J.util.C.getColix (argb);
return J.util.C.getColixTranslucent3 (J.util.C.getColix (argb), true, a / 255);
}, "~N");
