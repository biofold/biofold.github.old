...
return (J.minimize.Util.isNearZero (ab2, 1e-3) || J.minimize.Util.isNearZero (bc2, 1e-3) ? 0 : Math.acos ((ab2 + bc2 - ac2) / 2 / Math.sqrt (ab2 * bc2)));
}, "~A,~A,~A");
c$.isApprox = Clazz.defineMethod (c$, "isApprox", 
function (a, b, precision) {
return (J.minimize.Util.distance2 (a, b) <= precision * precision * Math.min (a.lengthSquared (), b.lengthSquared ()));
...
function (a, b) {
return J.minimize.Util.isNegligible (a, b, 1e-11);
}, "~N,~N");
...
function (a, b) {
return J.minimize.Util.isNear (a, b, 2e-6);
}, "~N,~N");
c$.isNear = Clazz.defineMethod (c$, "isNear", 
...
function (a) {
return J.minimize.Util.isNearZero (a, 2e-6);
}, "~N");
c$.isNearZero = Clazz.defineMethod (c$, "isNearZero", 
function (a, epsilon) {
