...
Clazz.load (["J.api.JmolScriptManager", "java.util.ArrayList"], "J.viewer.ScriptManager", ["java.lang.Boolean", "$.Thread", "J.api.Interface", "J.thread.CommandWatcherThread", "$.ScriptQueueThread", "J.util.Logger", "$.SB", "$.TextFormat"], function () {
...
return this.addScript (strScript, false, isQuiet && !this.viewer.getMessageStyleChime ());
}, "~S,~B,~B");
...
}if (isInsert || this.viewer.waitForMoveTo ()) {
...
Clazz.load (["J.api.JmolScriptEvaluator"], "J.script.ScriptEvaluator", ["java.lang.Boolean", "$.Float", "$.NullPointerException", "$.Short", "$.Thread", "java.util.ArrayList", "$.Hashtable", "J.atomdata.RadiusData", "J.constant.EnumAnimationMode", "$.EnumAxesMode", "$.EnumPalette", "$.EnumStereoMode", "$.EnumStructure", "$.EnumVdw", "J.i18n.GT", "J.io.JmolBinary", "J.modelset.Atom", "$.AtomCollection", "$.Bond", "$.Group", "$.LabelToken", "$.MeasurementData", "$.ModelCollection", "$.TickInfo", "J.script.SV", "$.ScriptCompiler", "$.ScriptContext", "$.ScriptException", "$.ScriptInterruption", "$.ScriptMathProcessor", "$.ScriptVariableInt", "$.T", "J.shape.Object2d", "J.util.ArrayUtil", "$.BS", "$.BSUtil", "$.BoxInfo", "$.C", "$.ColorEncoder", "$.ColorUtil", "$.Elements", "$.Escape", "$.GData", "$.JmolEdge", "$.Logger", "$.Matrix3f", "$.Matrix4f", "$.Measure", "$.MeshSurface", "$.P3", "$.Parser", "$.Point3fi", "$.Point4f", "$.Quaternion", "$.SB", "$.TextFormat", "$.V3", "J.viewer.ActionManager", "$.FileManager", "$.JC", "$.StateManager", "$.Viewer"], function () {
c$ = Clazz.decorateAsClass (function () {
...
for (var i = 0; i < xyz.length; i += 2) params.add (J.script.SV.newVariable (3, Float.$valueOf (0)).setName (xyz.substring (i, i + 1)));
...
case 1048614:
case 2:
rpn.addXNum ( new J.script.ScriptVariableInt (this.theToken.intValue));
break;
case 135266319:
...
resx.add (J.script.SV.newVariable (10, v));
}return resx;
}, "~O");
...
}, "J.util.BS,~N,J.util.P3,J.util.Point4f,~O,~O,~B,~N,~B");
...
this.contextVariables.put ("_breakval",  new J.script.ScriptVariableInt (2147483647));
...
this.popContext (false, false);
return v;
}, "J.api.JmolScriptFunction,~S,java.util.List,J.script.SV,~B,~B,~B");
...
}, $fz.isPrivate = true, $fz), "J.api.JmolScriptFunction,java.util.List,J.script.SV");
...
fixed[j] = J.script.T.t (2, (v).intValue (), v);
} else if (Clazz.instanceOf (v, Float)) {
fixed[j] = J.script.T.t (3, J.script.ScriptEvaluator.getFloatEncodedInt ("" + v), v);
...
} else if (Clazz.instanceOf (v, J.util.Point4f)) {
...
if (token.intValue != 2147483647) sb.appendI (token.intValue);
 else sb.append (J.modelset.Group.getSeqcodeString (J.script.ScriptEvaluator.getSeqCode (token)));
token = statement[++i];
sb.appendC (' ');
sb.append (inBrace ? "-" : "- ");
case 1048614:
...
case 1048613:
sb.appendC ('[');
sb.append (J.modelset.Group.getGroup3 (token.intValue));
sb.appendC (']');
continue;
case 1048612:
sb.appendC ('[');
...
break;
case 1048614:
if (isInMath) rpn.addXNum ( new J.script.ScriptVariableInt (instruction.intValue));
 else rpn.addXBs (this.getAtomBits (1048614,  new Integer (J.script.ScriptEvaluator.getSeqCode (instruction))));
break;
case 1048615:
if (isInMath) {
rpn.addXNum ( new J.script.ScriptVariableInt (instruction.intValue));
rpn.addOp (J.script.T.tokenMinus);
rpn.addXNum ( new J.script.ScriptVariableInt (code[++pc].intValue));
break;
...
return (instruction.intValue != 2147483647 ? J.modelset.Group.getSeqcode (instruction.intValue, ' ') : (instruction.value).intValue ());
...
case 9:
plane = J.util.Point4f.newPt (this.theToken.value);
break;
case 1048583:
var id = this.objectNameParameter (++i);
if (this.chk) return  new J.util.Point4f ();
...
J.util.Measure.getPlaneThroughPoints (points[0], points[1], points[2],  new J.util.V3 (), vAB, vAC, plane =  new J.util.Point4f ());
break;
case 23:
...
plane = J.util.Point4f.new4 (1, 0, 0, -this.floatParameter (i));
break;
case 1112541206:
if (!this.checkToken (++i) || this.getToken (i++).tok != 269484436) this.evalError ("y=?", null);
plane = J.util.Point4f.new4 (0, 1, 0, -this.floatParameter (i));
break;
case 1112541207:
if (!this.checkToken (++i) || this.getToken (i++).tok != 269484436) this.evalError ("z=?", null);
plane = J.util.Point4f.new4 (0, 0, 1, -this.floatParameter (i));
break;
case 1073741824:
case 4:
var str = this.parameterAsString (i);
if (str.equalsIgnoreCase ("xy")) return J.util.Point4f.new4 (0, 0, 1, 0);
if (str.equalsIgnoreCase ("xz")) return J.util.Point4f.new4 (0, 1, 0, 0);
if (str.equalsIgnoreCase ("yz")) return J.util.Point4f.new4 (1, 0, 0, 0);
this.iToken += 2;
break;
case 1048586:
if (!this.isPoint3f (i)) {
...
if (!this.chk) this.viewer.toCartesian (pt, !this.viewer.getFractionalRelative ());
}return pt;
}if (n == 4) {
if (this.coordinatesAreFractional) this.error (22);
var plane = J.util.Point4f.new4 (coord[0], coord[1], coord[2], coord[3]);
return plane;
}return coord;
...
case 0:
if (this.chk || !this.viewer.getMessageStyleChime ()) break;
...
v.set (J.script.SV.getVariable (bsOrList), false);
v.intValue = 1;
}} else {
v.intValue++;
...
if (this.chk) return;
if (tv == null) tv = (v == null ?  new J.script.ScriptVariableInt (0) : v);
t.value = tv.value;
...
if (this.isJS && floatSecondsTotal > 0 && this.viewer.waitForMoveTo ()) throw  new J.script.ScriptInterruption (this, "moveTo", 1);
return;
...
this.setShapeProperty (30, "position", J.util.P3.new3 (50, 50, 3.4028235E38));
this.setBooleanProperty ("navigationMode", true);
...
if (!this.chk) list.add ([Integer.$valueOf (554176526), Float.$valueOf (timeSec), Float.$valueOf (depth)]);
continue;
case 12289:
pt = this.centerParameter (++i);
i = this.iToken;
if (!this.chk) list.add ([Integer.$valueOf (135266320), Float.$valueOf (timeSec), pt]);
continue;
case 528432:
switch (this.getToken (++i).tok) {
case 1112541205:
...
if (!this.chk) list.add ([Integer.$valueOf (528432), Float.$valueOf (timeSec), rotAxis, Float.$valueOf (degrees)]);
continue;
case 4160:
var x = NaN;
var y = NaN;
...
pt = this.centerParameter (i);
i = this.iToken;
if (!this.chk) list.add ([Integer.$valueOf (4160), Float.$valueOf (timeSec), pt]);
continue;
}
}if (!this.chk) list.add ([Integer.$valueOf (269484210), Float.$valueOf (timeSec), Float.$valueOf (x), Float.$valueOf (y)]);
continue;
case 269484208:
continue;
case 1113200654:
...
if (!this.chk) list.add ([Integer.$valueOf (1073742084), Float.$valueOf (timeSec), path, theta, [indexStart, indexEnd]]);
continue;
}var v =  new java.util.ArrayList ();
...
if (!this.chk) list.add ([Integer.$valueOf (1073742084), Float.$valueOf (timeSec), path, theta, [0, 2147483647]]);
continue;
}default:
this.error (22);
}
...
if (isFrames) vAtomSets2.add (bss = [J.util.BSUtil.copy (bss[0]), bss[1]]);
bss[0].and (bsFrom);
}
if (isAtoms) {
...
}, "J.util.BS,J.util.BS,~S,java.util.List,java.util.List,J.util.Matrix4f,java.util.List,~B,~B");
...
for (var j = 0; j < b.length; j++) matches[j] = J.util.Escape.eB (b[j], asAtoms);
...
}if (!this.chk && shapeType == 26 && !this.mo (true)) return;
...
var data = null;
var bsSelected = (pal !== J.constant.EnumPalette.PROPERTY && pal !== J.constant.EnumPalette.VARIABLE || !this.viewer.isRangeSelected () ? null : this.viewer.getSelectionSet (false));
...
loadScript.append (" " + J.util.Escape.e (bsModels));
} else {
...
sOptions += " " + J.util.Escape.escapeAI (iArray);
tok = this.tokAt (i);
break;
}
var lattice = null;
...
loadScript.append (" COORD " + J.util.Escape.e (bs));
...
var preSelected = "; select " + J.util.Escape.e (bs) + ";\n ";
...
stateScript = "select " + J.util.Escape.e (bs) + ";\n ";
break;
case 1052714:
...
rotAxis.normalize ();
J.util.Measure.getPlaneThroughPoint (points[0], rotAxis, invPlane =  new J.util.Point4f ());
}q = J.util.Quaternion.newVA (rotAxis, endDegrees);
...
var pt;
var pts = (t).getList ();
for (var j = 0; j < pts.size (); j++) if ((pt = J.script.SV.ptValue (pts.get (j))) != null) data.add (pt);
 else return null;
...
this.checkLast (this.iToken);
if (!this.chk) this.viewer.setAtomCoordRelative (pt, bs);
return;
...
if (this.isJS && floatSecondsTotal > 0 && this.viewer.waitForMoveTo ()) throw  new J.script.ScriptInterruption (this, "zoomTo", 1);
}, $fz.isPrivate = true, $fz), "~B");
...
if (this.chk) continue;
this.getShapePropertyData (23, "getVertices", data);
value = data;
break;
case 10:
case 1048577:
...
if (!this.chk && propertyName != null) this.setShapeProperty (29, propertyName, propertyValue);
}, $fz.isPrivate = true, $fz));
...
if (J.viewer.ActionManager.getPickingStyle (str) < 0) this.errorStr2 (50, "SET PICKINGSTYLE " + type, str);
...
var tnew = (J.script.SV.newVariable (4, "")).set (tv, false);
...
this.viewer.setData (propertyName, [propertyName, (tv.tok == 7 ? J.script.SV.flistValue (tv, (tv.value).size () == bs.cardinality () ? bs.cardinality () : this.viewer.getAtomCount ()) : tv.asString ()), J.util.BSUtil.copy (bs),  new Integer (tv.tok == 7 ? 1 : 0)], this.viewer.getAtomCount (), 0, 0, tv.tok == 7 ? 2147483647 : -2147483648, 0);
return;
...
this.viewer.setData (key, [key, "" + vv, J.util.BSUtil.copy (this.viewer.getSelectionSet (false)),  new Integer (0)], this.viewer.getAtomCount (), 0, 0, -2147483648, 0);
return;
...
this.setStringProperty (key, J.util.Escape.eB (vv, false));
} else if (Clazz.instanceOf (vv, J.util.BS) || Clazz.instanceOf (vv, J.util.P3) || Clazz.instanceOf (vv, J.util.Point4f)) {
...
if (this.isJS && floatSecondsTotal > 0 && this.viewer.waitForMoveTo ()) throw  new J.script.ScriptInterruption (this, "restoreRotation", 1);
}return;
case 1073742077:
floatSecondsTotal = (this.slen > 3 ? this.floatParameter (3) : 0);
if (floatSecondsTotal < 0) this.error (22);
...
if (this.isJS && floatSecondsTotal > 0 && this.viewer.waitForMoveTo ()) throw  new J.script.ScriptInterruption (this, "restoreOrientation", 1);
}return;
case 1678770178:
...
if (this.chk) return "";
} else if (J.util.Parser.isOneOf (val.toLowerCase (), "png;pngj;pngt;jpg;jpeg;jpg64;jpeg64") && J.script.ScriptEvaluator.tokAtArray (pt + 1, args) == 2) {
quality = J.script.SV.iValue (this.tokenAt (++pt, args));
...
this.viewer.setAtomCoordRelative (J.util.P3.new3 (0, 0, 0), null);
...
break;
case 1073741828:
msg = J.util.Escape.escapeAP (this.viewer.getAdditionalHydrogens (null, true, true, null));
break;
case 1613758476:
...
case 135180:
if (!this.chk) msg = this.getShapeProperty (23, "jvxlDataXml");
break;
case 1183762:
...
sbCommand.append (" ").append (J.util.Escape.e (bs));
break;
}
idSeen = (this.theTok != 12291);
}
...
}if (params != null) sbCommand.append (" parameters ").append (J.util.Escape.e (params));
...
sbCommand.append (" ").appendO (this.theToken.value);
propertyValue = this.centerParameter (++i);
sbCommand.append (" ").append (J.util.Escape.e (propertyValue));
i = this.iToken;
break;
case 1679429641:
...
sbCommand.append (" intersection ").append (J.util.Escape.e (bsSelect)).append (" ").append (J.util.Escape.e (bs));
i = this.iToken;
...
sbCommand.append (" within ").appendF (distance).append (" ").append (bs == null ? J.util.Escape.eP (ptc) : J.util.Escape.e (bs));
}continue;
case 1073742083:
...
sbCommand.append (" parameters ").append (J.util.Escape.e (fparams));
break;
case 1716520973:
case 1073742190:
...
}if (!this.chk) sbCommand.append (" \"\" ").append (J.util.Escape.e (data));
} else {
this.getToken (++i);
if (!this.chk) {
...
sbCommand.append (" ignore ").append (J.util.Escape.e (bs2));
}if (surfaceObjectSeen || isMapped) {
sbCommand.append (" select " + J.util.Escape.e (propertyValue));
} else {
...
propertyName = "center";
propertyValue = this.centerParameter (++i);
sbCommand.append (" center " + J.util.Escape.e (propertyValue));
i = this.iToken;
break;
case 1073742147:
case 1766856708:
...
i = this.iToken;
sbCommand.append (" plane ").append (J.util.Escape.e (propertyValue));
break;
case 1073742138:
propertyName = "scale";
propertyValue = Float.$valueOf (this.floatParameter (++i));
...
i = this.iToken;
sbCommand.append (" ellipsoid ").append (J.util.Escape.e (propertyValue));
break;
} catch (e) {
if (Clazz.exceptionOf (e, J.script.ScriptException)) {
} else {
...
bs = this.atomExpressionAt (i);
sbCommand.append (" ellipsoid ").append (J.util.Escape.e (bs));
...
sbCommand.append (" discrete ").append (J.util.Escape.e (propertyValue));
i = this.iToken;
break;
case 1073741981:
...
sbCommand.append (J.util.Escape.e (propertyValue));
i = this.iToken;
}break;
case 1073741928:
...
propertyValue = this.getPoint4f (++i);
sbCommand.append (" eccentricity ").append (J.util.Escape.e (propertyValue));
i = this.iToken;
break;
case 1074790508:
sbCommand.append (" ed");
...
sbCommand.append (" ").append (J.util.Escape.e (pt4));
...
sbCommand.append (" ignore ").append (J.util.Escape.e (propertyValue));
i = this.iToken;
break;
case 1073741984:
...
propertyValue = this.getPoint4f (++i);
i = this.iToken;
sbCommand.append (" lobe ").append (J.util.Escape.e (propertyValue));
surfaceObjectSeen = true;
break;
case 1073742004:
case 1073742006:
...
propertyValue = this.getPoint4f (++i);
i = this.iToken;
sbCommand.append (" lp ").append (J.util.Escape.e (propertyValue));
surfaceObjectSeen = true;
break;
case 1052700:
...
sbCommand.append ("plane ").append (J.util.Escape.e (plane));
planeSeen = true;
plane = null;
...
propertyValue = this.getPoint4f (++i);
i = this.iToken;
sbCommand.append (" radical ").append (J.util.Escape.e (propertyValue));
break;
case 1073742028:
propertyName = "fixed";
propertyValue = Boolean.FALSE;
...
sbCommand.append (" phase ").append (J.util.Escape.e (propertyValue));
break;
case 1073742104:
case 1073742122:
propertyName = "resolution";
...
sbCommand.append (" within 2.0 ").append (J.util.Escape.e (bs));
...
sbCommand =  new J.util.SB ().append ("; isosurface map select ").append (J.util.Escape.e (bsSelect)).append (sbCommand.substring (16));
...
this.setShapeProperty (iShape, "finalize", (cmd.indexOf ("; isosurface map") == 0 ? "" : " select " + J.util.Escape.e (bsSelect) + " ") + cmd);
...
sarea = (doCalcArea ? "isosurfaceArea = " + (Clazz.instanceOf (area, Float) ? area : J.util.Escape.escapeAF (area)) : null);
svol = (doCalcVolume ? "isosurfaceVolume = " + (Clazz.instanceOf (volume, Float) ? volume : J.util.Escape.escapeAF (volume)) : null);
if (s == null) {
...
}, $fz.isPrivate = true, $fz), "java.util.List,~N,J.util.P3,J.util.BS,~B");
...
// 
Clazz.declarePackage ("J.script");
Clazz.load (["J.script.ScriptCompilationTokenParser", "java.util.ArrayList"], "J.script.ScriptCompiler", ["java.lang.Boolean", "$.Character", "$.Double", "$.Float", "java.util.Hashtable", "J.api.Interface", "J.i18n.GT", "J.io.JmolBinary", "J.modelset.Bond", "$.Group", "J.script.ContextToken", "$.SV", "$.ScriptContext", "$.ScriptEvaluator", "$.ScriptFlowContext", "$.ScriptFunction", "$.T", "J.util.ArrayUtil", "$.BS", "$.Escape", "$.Logger", "$.Parser", "$.SB", "$.TextFormat", "J.viewer.Viewer"], function () {
c$ = Clazz.decorateAsClass (function () {
this.filename = null;
this.isSilent = false;
this.contextVariables = null;
...
this.iBrace++;
var t =  new J.script.ContextToken (266280, 0, "{");
...
if ((this.theToken = J.script.T.getTokenFromName (ident)) == null && (this.theToken = J.script.T.getTokenFromName (identLC)) != null) this.theToken = J.script.T.t (this.theToken.tok, this.theToken.intValue, ident);
} else {
ident = identLC;
...
this.addTokenToPrefix (J.script.T.o (3,  new Float (value)));
return 2;
...
this.addTokenToPrefix (J.script.T.t (3, intValue,  new Float (value)));
return 2;
...
}var seqcode = J.modelset.Group.getSeqcode (seqNum, insertionCode);
this.addTokenToPrefix (J.script.T.t (5, seqcode, "seqcode"));
return 2;
} catch (nfe) {
...
this.addTokenToPrefix (J.script.T.t (2, val, intString));
return 2;
...
this.replaceCommand (this.flowContext.token =  new J.script.ContextToken (102402, "elseif"));
...
this.ltoken.set (0, this.setCommand (J.script.T.t (1073741824, 0, token.value)));
this.setBraceCount = 0;
break;
...
this.addTokenToPrefix (this.setCommand ( new J.script.ContextToken (266278, 0, "}")));
this.isEndOfCommand = true;
return 2;
...
isEnd = true;
if (this.flowContext.token.tok != 135368713 && this.flowContext.token.tok != 102436 && this.flowContext.token.tok != 364558) this.setCommand (J.script.T.t (this.tokCommand, (this.flowContext.ptDefault > 0 ? this.flowContext.ptDefault : -this.flowContext.pt0), ident));
break;
case 364558:
case 102412:
break;
...
this.setCommand (J.script.T.t (this.tokCommand, f.pt0, ident));
break;
case 102413:
...
var ct =  new J.script.ContextToken (this.tokCommand, this.tokenCommand.value);
this.setCommand (ct);
switch (this.tokCommand) {
case 364558:
...
// 
Clazz.declarePackage ("J.script");
Clazz.load (null, "J.script.ScriptMathProcessor", ["java.lang.Float", "java.util.ArrayList", "$.Arrays", "$.Date", "$.Hashtable", "java.util.regex.Pattern", "J.atomdata.RadiusData", "J.constant.EnumVdw", "J.modelset.Bond", "$.MeasurementData", "J.script.SV", "$.ScriptEvaluator", "$.ScriptVariableInt", "$.T", "J.util.ArrayUtil", "$.AxisAngle4f", "$.BS", "$.BSUtil", "$.ColorEncoder", "$.ColorUtil", "$.Escape", "$.JmolMolecule", "$.Logger", "$.Matrix3f", "$.Matrix4f", "$.Measure", "$.P3", "$.Parser", "$.Point3fi", "$.Point4f", "$.Quaternion", "$.SB", "$.TextFormat", "$.V3"], function () {
c$ = Clazz.decorateAsClass (function () {
...
for (var i = 0; i <= this.xPt; i++) result.add (J.script.SV.selectItemVar (this.xStack[i]));
...
($fz = function (x) {
this.putX ( new J.script.ScriptVariableInt (x));
return this.wasX = true;
}, $fz.isPrivate = true, $fz), "~N");
...
x =  new J.script.ScriptVariableInt (-x.intValue);
}break;
case 3:
var f = (x.value).floatValue ();
if (f < 0 || f == 0 && 1 / f == -Infinity) {
this.addOp (J.script.T.tokenMinus);
x = J.script.SV.newVariable (3,  new Float (-f));
}break;
}
this.putX (x);
return this.wasX = true;
...
this.xStack[this.xPt] = J.script.SV.newVariable (4, "").set (x, false);
...
y.add (last);
y.add (count);
counts.add (J.script.SV.getVariableList (y));
}count =  new J.script.ScriptVariableInt (1);
last = a;
}
...
($fz = function (x) {
if (this.isSyntaxCheck) return  new J.util.Point4f ();
switch (x.tok) {
case 9:
return x.value;
case 7:
...
return (Clazz.instanceOf (pt, J.util.Point4f) ? pt : null);
case 10:
break;
}
return null;
...
if (Clazz.instanceOf (pt, J.util.Point4f)) return this.addXPt4 (pt);
...
return this.addXPt4 (J.util.Point4f.new4 (norm.x, norm.y, norm.z, nd));
}
}
...
return this.addXPt4 (J.util.Point4f.new4 (x, y, z, w));
}, $fz.isPrivate = true, $fz), "~A,~N");
...
case 4:
return this.addXPt4 (J.util.Point4f.new4 (args[0].asFloat (), args[1].asFloat (), args[2].asFloat (), args[3].asFloat ()));
}
return false;
}, $fz.isPrivate = true, $fz), "~A");
...
if (!(Clazz.instanceOf (v, J.util.Point4f))) return false;
p4 = v;
...
break;
case 4:
if (tok == 135270417) p4 = J.util.Point4f.new4 (J.script.SV.fValue (args[1]), J.script.SV.fValue (args[2]), J.script.SV.fValue (args[3]), J.script.SV.fValue (args[0]));
...
i = args.length - 1;
if (Clazz.instanceOf (args[i].value, J.util.Point4f)) {
plane = args[i].value;
...
return this.addXPt (pt);
case 9:
pt4 = J.util.Point4f.newPt (x2.value);
pt4.scale (-1.0);
...
if (x2.tok == 9) {
var v4 = J.util.Point4f.newPt (x2.value);
...
return this.addXPt4 (J.util.Point4f.new4 (ax.x, ax.y, ax.z, (ax.angle * 180 / 3.141592653589793)));
case -9:
...
if (f2 == 0) return this.addXPt4 (J.util.Point4f.new4 (NaN, NaN, NaN, NaN));
...
case 9:
if (f == 0) return this.addXPt4 (J.util.Point4f.new4 (NaN, NaN, NaN, NaN));
...
if (Clazz.instanceOf (pt, J.util.Point4f)) return J.script.ScriptMathProcessor.getMinMaxQuaternion (sv, tok);
break;
}} else {
break;
}var sum;
